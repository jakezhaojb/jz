...
unit_test_SpatialMlpPoolingWTA = function()
   print("--- Input --- ")
   --x = torch.rand(1,3,32,32)
   x = torch.rand(1,3,8,8)
   print(x)
   a = nn.Sequential()
   --a:add(jz.SpatialMlpPoolingWTA(3,3,8,8))
   a:add(jz.SpatialMlpPoolingWTA(3,3,2,2))
   print("--- weight ---")
   print(a:get(1).weight)
   print("--- output benchmark --- ")
   print(a:get(1).weight[1]:sum() .. ' ' .. a:get(1).weight[2]:sum())
   ...
   y = a:forward(x)
   print(y)
   dx_cpu = a:get(1).dx:clone()
   dy_cpu = a:get(1).dy:clone()
   randGrad = torch.rand(y:size()):typeAs(y)
   -- Update parameters
   _, grad = a:getParameters()
   a:zeroGradParameters()
   ...
   y = a:forward(xx)
   print(y)
   dx_gpu = a:get(1).dx:clone()
   dy_gpu = a:get(1).dy:clone()
   randGrad = randGrad:cuda()
   -- Update parameters
   _, grad = a:getParameters()
   a:zeroGradParameters()
   ...
   print(grad:add(grad_cpu:cuda():mul(-1)):norm())
   print(dfdx:add(dfdx_cpu:cuda():mul(-1)):norm())
   print(dx_gpu:add(dx_cpu:cuda():mul(-1)):norm())
   print(dy_gpu:add(dy_cpu:cuda():mul(-1)):norm())
end
...
unit_test_SpatialMlpPoolingWTA()
